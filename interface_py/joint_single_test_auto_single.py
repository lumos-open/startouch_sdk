...
class SafeArmController:
    ...
    def _cleanup(self):
        ...
        if self.initialized and self.arm_controller:
            try:
                ...
                pos = [0, -0.35, 0, 1.6, 0, 0]         # mot2 up & mot4 up
    ...
    def run(self):
        ...
        try:
            ...
            while True:
                #pos = [0, 1.54, 3, -1.3, 0, 0]        # 垂直 & mot4 down
                #self.arm_controller.set_joint(pos, tf=2)
                #pos = [2.7, 1.54, 3, 1.6, 1.6, -2.7]   # mot1 right & mot3 up & mot4 up & mot5 left &mot6 cw
                #self.arm_controller.set_joint(pos, tf=2)
                #pos = [-2.7, 1.54, 3, 0, -1.6, 2.7]    # mot1 left & mot5 left &mot6 ccw
                #self.arm_controller.set_joint(pos, tf=3)
                #self.arm_controller.setGripperPosition(1)
                #self.arm_controller.go_home()               # mot3 down
                #self.arm_controller.setGripperPosition(0)
                pos = [0, 3.3, 1.5, 0, 0, 0]           # mot2 down
                self.arm_controller.set_joint(pos, tf=1)
                pos = [0, -0.18, 0, 1.6, 0, 0]         # mot2 up & mot4 up
                self.arm_controller.set_joint(pos, tf=1)
                count = count+1
                print(f"count = {count}")
        except Exception as e:

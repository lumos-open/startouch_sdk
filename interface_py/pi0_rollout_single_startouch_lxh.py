...
主要功能：
    ...
    4. 解析单臂 7 维动作（xyz + RPY + gripper），转为弧度后发送给 startouch 执行。
...
def interp_and_move_single(
    ...
    for i in range(steps + 1):
        arm.set_end_effector_pose_euler_raw(
            ...
            euler=euler_arr_rad[i].tolist(),
        )
        time.sleep(dt)

    arm.setGripperPosition(g)
        ...
        while True:
            ...
            for step_idx, action in enumerate(action_slice, start=lo):
                ...
                if traj_log_fp is not None:
                    action_vals = np.asarray(action, dtype=np.float64).tolist()
                    traj_log_fp.write(
                    ...
                    traj_log_fp.flush()
                cur_pos, cur_euler = tgt_pos, tgt_euler_rad
            infer_idx += 1

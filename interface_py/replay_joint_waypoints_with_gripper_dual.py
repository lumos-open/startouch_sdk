...
def move_joint_waypoints_with_gripper_interruptible(arm, waypoints, gripper_positions, **kwargs):
    return run_interruptible_arm_call(
...
def clamp_width_to_gripper_norm(width):
    distance = clamp_width_to_distance(width)
    if GRIPPER_MAX_DISTANCE_M <= 0.0:
        raise ValueError("GRIPPER_MAX_DISTANCE_M must be > 0")
    return float(np.clip(distance / GRIPPER_MAX_DISTANCE_M, 0.0, 1.0))
...
def run_dual_replay(left_args, right_args):
    ...
    def target(name, args):
        try:
            results[name] = (True, replay_joint_waypoints_with_gripper_original_timing(*args))
        except BaseException as e:
...
def solve_all_poses_to_joints(arm, poses, q_seed, label):
    joint_waypoints = []
    q_current = list(q_seed)
    for idx, pose in enumerate(poses):
        q_current = solve_pose_to_joints(arm, pose, q_current, f"{label}[{idx}]")
        joint_waypoints.append(q_current)
    return joint_waypoints


def estimate_equal_spacing_time_sec(source_timestamps, sampled_count, interval, speed_rate):
    if sampled_count <= 1:
        return 0.0
    source_timestamps = np.asarray(source_timestamps, dtype=float)
    diffs = np.diff(source_timestamps)
    diffs = diffs[np.isfinite(diffs) & (diffs > 0)]
    source_dt = float(np.median(diffs)) if len(diffs) else 0.01
    return float((sampled_count - 1) * source_dt * interval / speed_rate)


def replay_joint_waypoints_with_gripper_original_timing(
    arm,
    speed_rate,
    target_pose,
    target_clamp_width,
    ...
    total_time = estimate_equal_spacing_time_sec(
        source_timestamps,
        len(sampled_pose),
        interval,
        speed_rate,
    ...
    print(
        f"开始 joint waypoints with gripper 轨迹复现 (interval={interval}): "
        f"原始pose点数={len(source_pose)}, 抽帧后pose点数={len(sampled_pose)}, "
        f"抽帧后首尾时间差: {raw_total_time:.2f} 秒, "
        f"等距时间戳执行时长: {total_time:.2f} 秒"
    )
    ...
    print("开始对抽帧后所有 pose 做 IK...")
    joint_waypoints = solve_all_poses_to_joints(
        arm,
        sampled_pose,
        initial_joints,
        "sampled_pose",
    )
    q_first = joint_waypoints[0]
    q_last = joint_waypoints[-1]
    gripper_positions = [clamp_width_to_gripper_norm(width) for width in sampled_clamp]
    print(f"joint waypoints 起点IK: {q_first}")
    print(f"joint waypoints 终点IK: {q_last}")
    print(f"IK完成: joint_waypoints={len(joint_waypoints)}, gripper_points={len(gripper_positions)}")
    ...
    if len(sampled_pose) == 1:
        print("轨迹只有 1 个采样点，跳过 joint waypoint 轨迹。")
        motion_duration = 0.0
    else:
        replay_joint_waypoints = joint_waypoints[1:]
        replay_gripper_positions = gripper_positions[1:]
        print(
            "下发 joint waypoints with gripper: "
            f"waypoints={len(replay_joint_waypoints)}, "
            f"time_sec={total_time:.3f}, "
            "当前关节状态作为第 0 个等距时间戳点"
        )
        motion_duration = move_joint_waypoints_with_gripper_interruptible(
            arm,
            replay_joint_waypoints,
            replay_gripper_positions,
            time_sec=total_time,
        )
        if len(sampled_clamp) > 0:
            arm.setGripperDistance(clamp_width_to_distance(sampled_clamp[-1]), 10, 0.5)
        print(f"joint waypoints with gripper 轨迹回放完成: planned_duration_s={motion_duration:.3f}")
...
if __name__ == "__main__":  
    ...
    try:
        ...
        print(f"\n[ERROR] joint waypoints with gripper 轨迹回放失败: {e}")
        traceback.print_exc()
    finally:
        if config["StarTouch"]["enable"]:
            cleanup_active_arms()

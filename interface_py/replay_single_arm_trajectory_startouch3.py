...
def smooth_interpolate_positions(start_pos, end_pos, start_quat, end_quat, total_time, dt=0.0025, accel_time=0.2):
    ...
    # 计算最小时间（基于最大速度和加速度）
    max_vel = 0.05  # m/s - 降低最大速度
    max_accel = 0.1  # m/s² - 降低最大加速度
...
def smooth_interpolate_and_move(arm: SingleArm, start_pos, start_quat_wxyz, target_pos, target_quat_wxyz, total_time=2.0, dt=0.0025):
    """使用S曲线规划的平滑移动"""
    positions, quats, times = smooth_interpolate_positions(
        start_pos, target_pos, start_quat_wxyz, target_quat_wxyz, total_time, dt
    )
...
def return_to_home_with_interp(arm: SingleArm, home_pos, home_quat_wxyz, step_size=0.01, dt=0.04):
    cur_pos, cur_quat = arm.get_ee_pose_quat()
    return smooth_interpolate_and_move(arm, cur_pos.tolist(), cur_quat.tolist(), home_pos, home_quat_wxyz, total_time=5.0, dt=0.01)
...
def main():
    ...
    poses_left, timestamps_left = trajectory_to_startouch(left_traj, T_base_to_local)
    n_steps = len(poses_left)
    print(f'轨迹步数: {n_steps}')

    # 当前阶段仅控制前6个自由度，不发送夹爪命令
    ...
    if args.dry_run:
        if args.debug_interp:
            ...
            for i in range(min(5, n_steps)):
                print(f'Step {i} pos={poses_left[i][0]} quat={poses_left[i][1]}')
            return
        ...
        if not move_to_first_with_interp(left_arm, p0, q0, step_size=args.interp_step_size, dt=args.dt):
            ...
            with tqdm(total=n_steps, desc='复现轨迹', unit='步') as pbar:
                ...
                for i in range(n_steps):
                    ...
                    # 根据原始轨迹时间戳计算正确的sleep时间
                    if i < n_steps - 1:
                        dt_actual = timestamps_left[i + 1] - timestamps_left[i]
                    else:
                        dt_actual = args.dt
                    
                    time.sleep(dt_actual)

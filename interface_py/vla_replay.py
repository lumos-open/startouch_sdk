...
# "setjointwaypointwithgripper": VLA pose -> IK at logged frames -> joint waypoint smoothing.
# "movepwithgripper": VLA pose -> C++ MoveP Cartesian smoothing/blending -> IK.
EXECUTION_MODE = "setjointwaypointwithgripper"
...
MOVE_TO_FIRST_POSE_SPEED_PERCENT = 3
...
PLOT_JOINT_OUT = Path("vla_replay_joint_cmd_vs_feedback.png")
PLOT_TOOL_OUT = Path("vla_replay_tool_cmd_vs_feedback.png")
...
def command_samples_to_array(arm, motion_result: MotionResult, tcp_offset_xyz):
    ...
    # Bottom-layer command samples are relative to actual motion start. The blocking call
    # returns after trajectory completion, so this removes pre-execution planning time.
    motion_start = motion_result.call_end_time - motion_result.duration
    out = arr[:, :15].copy()
...
def original_cmd_points_to_array(group, command_arr, execution_mode, tcp_offset_xyz, joint_points=None):
    ...
    if execution_mode == "setjointwaypointwithgripper":
...
def run_joint_waypoints_with_gripper(arm, joint_points, gripper_positions, time_sec, speed_percent):
    return arm.move_joint_waypoints_with_gripper(
        joint_points,
        gripper_positions,
        time_sec=time_sec,
...
def main():
    ...
    if EXECUTION_MODE not in ("setjointwaypointwithgripper", "movepwithgripper"):
        raise ValueError("EXECUTION_MODE must be 'setjointwaypointwithgripper' or 'movepwithgripper'")
        ...
        for group_idx, (start, group) in enumerate(groups, 1):
            ...
            if use_time_mode:
                ...
                if EXECUTION_MODE == "setjointwaypointwithgripper":
                    ...
                    thread, result = run_motion_thread(
                        run_joint_waypoints_with_gripper,
                        arm,
                        joint_points,
